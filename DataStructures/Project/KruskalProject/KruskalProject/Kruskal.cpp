...
vector <Edge> kruskal()
{
	...
	int count = 0;
	int i = 0; //iterator
	UnionFind ob(edges.size()); //instantiate the object of UnionFind holding all the egdes
	while (count < (num_vertices - 1)) {  /* count is the number of edges added so far */

		if (ob.FindSet(edges[i].From()) != ob.FindSet(edges[i].To()))
		{ //if they are not equal, make them equal and set them in the set together
			temp.push_back(edges[i]); //put unequal into temp vector
			count = count + 1;
			ob.Union(edges[i].From(), edges[i].To());
		}
		i++;
	}
	return temp; //return the vector to main
}
